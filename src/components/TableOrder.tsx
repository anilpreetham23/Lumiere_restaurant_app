...
import {
  ...
  Check, Loader2, ChefHat, UtensilsCrossed,
} from "lucide-react";
...
export default function TableOrder({
  ...
  async function payOnline() {
    ...
    const res = await startBillPayment(token);
        ...
        {orders.length > 0 && (
          <section className="mb-6">
            ...
            <div className="space-y-3">
              {orders.map((o) => <OrderCard key={o.id} order={o} menuMap={menuMap} />)}
            </div>
            ...
            {!effReceipt && total > 0 && (
              <div className="mt-3 grid gap-2">
                ...
                >
                  ...
                  {payBusy ? "Redirecting to payment…" : `Pay online · ${money(total)}`}
                </button>
...
function OrderCard({ order, menuMap }: { order: SessionOrder; menuMap: Record<string, MenuItem> }) {
